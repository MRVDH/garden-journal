...
from what "today" is, so both refresh at local midnight. The set of plants
...
class PlantEntity(Entity):
    ...
    async def async_added_to_hass(self) -> None:
        """Recompute state at local midnight, when today rolls over."""
    ...
    def _windows(self) -> list[Window] | None:
        """Resolve this plant's effective windows, or None on a repair case."""
        return resolve_windows(self._data, self._resolver)

    def _care(self) -> list[Care]:
        """Resolve this plant's continuous-care seasons, empty when it has none."""
